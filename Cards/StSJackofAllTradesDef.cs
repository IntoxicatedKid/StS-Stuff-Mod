...
namespace StSStuffMod.Cards
{
    ...
    public sealed class StSJackofAllTrades : Card
    {
        ...
        {
            List<Card> list = Battle.RollCardsWithoutManaLimit(new CardWeightTable(RarityWeightTable.BattleCard, OwnerWeightTable.Valid, CardTypeWeightTable.CanBeLoot), Value1, (config) => config.Colors.Contains(ManaColor.Colorless) && config.Id != Id).ToList();
            if (list.Count > 0)
            {
